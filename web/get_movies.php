...
// get all jobs
$result = mysql_query("SELECT * FROM allproducts WHERE type='Movie'");
...
if (mysql_num_rows($result))
{
	...
	$response1 = array();
	while($AllMovies = mysql_fetch_array($result))
	{
		// temp user array
		$movies = array();
		$movies["id"] = $AllMovies["id"];
		$movies["type"] = $AllMovies["type"];
		$movies["name"] = $AllMovies["name"];
		$movies["category"] = $AllMovies["category"];
		$movies["amount"] = $AllMovies["amount"];
